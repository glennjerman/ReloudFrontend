    ...
    return (
        open && 
            <div className="w-full h-30 fixed bottom-0 left-0 bg-lightShade flex flex-row">
                ...
                <h1 className="w-full p-2 text-center">{decodeURIComponent(src ? new URL(src).pathname.split('/').pop() : '').replace('.mp3', '')}</h1>
                    <div className="w-full flex-row flex justify-center">
                    <p className="text-sm mr-1 my-auto font-light">{formatTime(currentTime)}</p>
                    <progress value={currentTime} max={duration} className="audio-progress"></progress>
                    <p className="text-sm ml-1 my-auto font-light">{formatTime(duration)}</p>
                    </div>
                    <button onClick={togglePlay}><img className="w-7 h-7 mx-auto" src={isPlaying ? '/img/pause.png' : '/img/play.png'}></img></button>
